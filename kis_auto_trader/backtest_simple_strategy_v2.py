"""간단한 거래 전략 백테스트 (수정 버전)

전날(di-1)의 신호로 당일(di)의 수익을 예측합니다.

전략:
- 전날 약세 신호 + OVN 음수 → 당일 BUY (+0.592% 기대)
- 전날 약세 신호 + OVN 양수 → 당일 HOLD
- 전날 강세 신호 → 당일 HOLD (손실 회피)
"""
import csv
...
def calculate_ovn(panel, picks, di):
    ...
    for code in picks:
        ...
        if di - 1 in s.pos and di in s.pos:
            prev_close = s.close[s.pos[di - 1]]  # 전일 종가
            curr_open = s.open[s.pos[di]]  # 당일 시가
            if prev_close > 0:
...
def get_yesterday_daily_return(panel, picks, di):
    """전일(di-1)의 당일 수익률."""
    if di < 1:
        return None
...
def get_today_daily_return(panel, picks, di):
    """당일(di)의 당일 수익률."""
    rets = []
...
def simulate_strategy(dates, panel, rebal_interval=20, lookback=250):
    ...
    for di in rebal_dates:
        ...
        # 종목 선택 (전일 기준)
        di_prev = di - 1
        cand = S.universe_at(panel, di_prev, S.UNIVERSE_SIZE, S.MIN_PRICE)
        scored = [(S.factor_score(panel, c, di_prev, "low_vol"), c) for c in cand]
        ...
        # 전일 신호 판정 (전일의 당일 수익률로)
        yesterday_return = get_yesterday_daily_return(panel, picks, di)
        if yesterday_return is None:
            continue

        yesterday_signal = "bearish" if yesterday_return < 0 else "bullish"
        ...
        if yesterday_signal == "bearish":
            ...
            decision = "HOLD"
            confidence = 55
            expected_return = -0.0022  # -0.22%

        # 당일 실제 수익률
        actual_return = get_today_daily_return(panel, picks, di)
...
def analyze_results(trades):
    """결과 분석."""
    print("\n" + "=" * 120)
    print("📊 모의 투자 시뮬레이션 결과 (v2)")
    print("=" * 120)

    # 전체 거래
    print(f"\n[전체 거래]")
    print(f"  총 거래 기간: {len(trades)}개 리밸런스 포인트")

    # 의사결정별 분석
    buy_trades = [t for t in trades if t["decision"] == "BUY"]
    hold_trades = [t for t in trades if t["decision"] == "HOLD"]

    print(f"  BUY 신호: {len(buy_trades)}회")
    print(f"  HOLD 신호: {len(hold_trades)}회")

    # BUY 거래 분석
    print(f"\n[BUY 거래 분석 ({len(buy_trades)}회)]")
    if buy_trades:
        ...
        if buy_actual:
            ...
            print(f"  누적 수익률: {sum(buy_actual)*100:+.3f}%")
    ...
    # 약세 신호
    bearish_trades = [t for t in trades if t["yesterday_signal"] == "bearish"]
    if bearish_trades:
        print(f"\n  약세 신호 ({len(bearish_trades)}회):")
        ...
        if bearish_actual:
            ...
            print(f"    누적 수익률: {sum(bearish_actual)*100:+.3f}%")
            ...
            if bearish_buy:
                ...
                if bb_actual:
                    bb_wr = sum(1 for r in bb_actual if r > 0) / len(bb_actual) * 100
                    print(f"      └ BUY ({len(bb_actual)}회): {st.mean(bb_actual)*100:+.3f}% ({bb_wr:.1f}% 승률)")

            if bearish_hold:
                bh_actual = [t["actual_return"] for t in bearish_hold if t["actual_return"] is not None]
                if bh_actual:
                    bh_wr = sum(1 for r in bh_actual if r > 0) / len(bh_actual) * 100
                    print(f"      └ HOLD ({len(bh_actual)}회): {st.mean(bh_actual)*100:+.3f}% ({bh_wr:.1f}% 승률)")

    # 강세 신호
    bullish_trades = [t for t in trades if t["yesterday_signal"] == "bullish"]
    if bullish_trades:
        print(f"\n  강세 신호 ({len(bullish_trades)}회):")
        ...
        if bullish_actual:
            ...
            print(f"    누적 수익률: {sum(bullish_actual)*100:+.3f}%")
...
def save_backtest_report(trades):
    ...
    # CSV 저장
    csv_file = data_dir / "backtest_results_v2.csv"

    fieldnames = [
        "date", "yesterday_signal", "yesterday_return", "ovn", "decision",
    ...
    try:
        with open(csv_file, 'w', encoding='utf-8-sig', newline='') as f:
            ...
            for trade in trades:
                row = {
                    "date": trade["date"],
                    "yesterday_signal": trade["yesterday_signal"],
                    "yesterday_return": f"{trade['yesterday_return']:.6f}",
...
def main():
    ...
    print("=" * 120)
    print("\n이것은 모의 투자 시뮬레이션입니다.")
    print("- 과거 데이터 기반 (회계귀납적 분석)")
    print("- 실제 거래 시 슬리피지, 수수료, 시장 충격 등 변수 존재")
    print("- Phase 1 수집 데이터(실제 거래)로 재검증 필수")
