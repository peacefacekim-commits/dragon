...
실제 나라배움터 로그인 없이, 완료 감지 → 다음 강의 자동 전환을 눈으로 먼저 볼 수
있는 모의 강의 3개를 `demo/` 폴더에 만들어뒀습니다. 개인정보도 없고 로그인도
필요 없어서 발표·시연 자리에서 보여주기에 안전합니다.

1. **`설치.bat`** 을 처음 한 번 실행해둡니다.
2. **`시연_실행.bat`** 을 더블클릭합니다.
3. 브라우저가 뜨면 터미널에서 Enter 를 누릅니다. 각 강의는 60초 카운트다운 후
   "수강완료"로 바뀌고, 자동으로 다음 강의로 넘어갑니다. 3강까지 끝나면 "다음
   강의" 버튼이 없으니 자동화가 거기서 자연스럽게 멈춥니다 — 팝업이 없어도
   "더 이상 열 게 없으면 멈춘다"는 안전장치를 보여줄 수 있는 지점입니다.

이 모드는 `autoplay.py --config demo/demo_config.json --start-file demo/lecture1.html`
와 같습니다. `--start-file` 은 `config.json` 의 `start_url` 대신 로컬 HTML 파일을
열 때 씁니다.
...
> ⚠️ **주의**: `completion_texts` 는 화면 전체 텍스트에서 찾기 때문에, "완료되면
> 수강완료로 표시됩니다" 같은 안내 문구에도 그 단어가 들어있으면 아직 안 끝났는데
> 완료로 오인식할 수 있습니다. (실제로 이 저장소의 모의 강의를 만들다가 이 문제가
> 나서 고쳤습니다.) 가능하면 완료 배지 요소 하나를 콕 집은 `completion_selectors`
> 를 쓰는 게 더 안전합니다.
